...
(function () {
  ...
  S.win = {
    ...
    render(ctx) {
      ...
      // the player, watching it, facing the sun
      BC.gfx.actor(ctx, sunX - 8, horizon - 16, 'up', 0, { shirt: '#c0444f', hair: '#2f2218' });

      // words
      BC.rect(ctx, 0, horizon + 18, W, 64, 'rgba(8,8,18,0.55)');
      BC.text(ctx, 'THE LOOP LETS GO', W / 2, horizon + 22, { color: '#ffe27a', size: 13, align: 'center' });
      BC.text(ctx, '2 AM came... and the night just kept going.', W / 2, horizon + 40, { color: '#fff', size: 8, align: 'center' });
      BC.text(ctx, "Sun's up. You're not new here anymore - you're a regular.", W / 2, horizon + 52, { color: '#cfe', size: 8, align: 'center' });
      BC.text(ctx, 'Cleared in ' + g.meta.loops + ' night' + (g.meta.loops === 1 ? '' : 's') + '   -   Wins: ' + g.meta.wins, W / 2, horizon + 66, { color: '#9ab', size: 8, align: 'center' });
      if (t > 1.6 && (t % 1.0) < 0.6) BC.text(ctx, 'Press Z for another go', W / 2, H - 12, { color: '#fff', size: 8, align: 'center' });
    }
  };
})();
